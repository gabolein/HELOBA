...
struct in_addr get_frequency_multicast_addr(uint16_t frequency) {
  ...
  struct in_addr out;
  snprintf(multicast_addr, IPADDR_MAX_LEN, MULTICAST_FMT, (frequency & 0xFF00) >> 8, (frequency & 0x00FF) >> 0);
...
bool virtual_change_frequency(uint16_t frequency) {
  ...
  if (setsockopt(virt_fd,IPPROTO_IP,IP_ADD_MEMBERSHIP, &mreq,sizeof(mreq)) < 0) {
    fprintf(stderr, "Couldn't join multicast group for frequency %u", frequency);
...
bool virtual_transport_initialize() {
  ...
  if (setsockopt(virt_fd, IPPROTO_IP, IP_MULTICAST_LOOP, NULL, 0) < 0) {
    fprintf(stderr, "Couldn't enable multicast on loopback interface!\n");
    close(virt_fd);
    return false;
  }

  if (setsockopt(virt_fd, IPPROTO_IP, SO_REUSEADDR, NULL, 0)) {
    fprintf(stderr, "Couldn't get kernel to immediately reuse address after program terrmination.\n");
    close(virt_fd);
    return false;
  }
...
bool virtual_send_packet(uint8_t *buffer, unsigned length) {
  if (send(virt_fd, buffer, length, 0) < 0) {
    fprintf(stderr, "Couldn't send %u bytes to fd=%i", length, virt_fd);
    return false;
  }
